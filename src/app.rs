...
use gpui::{
    div, prelude::*, px, rgb, AnyElement, Context, FontWeight, Point, Render, ScrollStrategy,
    UniformListScrollHandle, Window,
};

use crate::analysis::{build_flow_graph, layout_graph, DataFlowGraph, GraphLayout};
use crate::git::{load_diff_lines_for_file, load_repo_snapshot, DiffLine, RepoSnapshot};
use crate::ui::diff_view::render_diff_view;
use crate::ui::file_tree::{build_file_tree, directory_id, FileTreeNode};
use crate::ui::graph_view::{render_graph_view, GraphViewState};
...
impl ReviewApp {
    ...
    fn ensure_flow_graph_loaded(&mut self) {
        ...
        if !path.ends_with(".rs") {
            self.flow_graphs
                .insert(path.clone(), DataFlowGraph::empty());
            self.flow_layouts.insert(
                path.clone(),
                GraphLayout {
                    nodes: Vec::new(),
                    edges: Vec::new(),
    ...
    fn render_file_list(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let body = if self.repo.files.is_empty() {
            ...
            div()
                ...
                .children(
                    tree.into_iter()
                        .map(|node| self.render_file_tree_node(&node, 0, cx)),
                )
        };
...
impl Render for ReviewApp {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let current_flow_layout = selected_file.and_then(|f| self.flow_layouts.get(&f.path));
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    ...
                                    .rounded_full()
                                    .border_1()
                                    .border_color(if self.show_data_flow {
                                        rgb(0x4f8cff)
                                    } else {
                                        rgb(0x22304f)
                                    })
                                    .bg(if self.show_data_flow {
                                        rgb(0x172544)
                                    } else {
                                        rgb(0x10182b)
                                    })
