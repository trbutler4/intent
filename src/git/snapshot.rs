...
use super::diff::{build_untracked_diff, fallback_diff_for_status, parse_unified_diff, DiffLine};
use super::status::{parse_name_status, parse_porcelain_status, StatusEntry};
...
fn load_last_commit_changes(repo_path: &Path) -> Result<Vec<FileChange>, String> {
    ...
    let name_status = if commit_count > 1 {
        ...
        run_git(
            repo_path,
            &["show", "--format=", "--name-status", "--root", "HEAD"],
        )?
    };
...
fn load_last_commit_diff_stats(
    ...
    if has_parent {
        run_git(repo_path, &["show", "--format=", "--numstat", "HEAD"])
            .map(|output| parse_numstat_output(&output))
    } else {
        run_git(
            repo_path,
            &["show", "--format=", "--numstat", "--root", "HEAD"],
        )
        .map(|output| parse_numstat_output(&output))
    }
}
