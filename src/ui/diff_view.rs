use gpui::{
    div, prelude::*, px, rgb, uniform_list, AnyElement, App, ClickEvent, FontWeight, Hsla,
    ListHorizontalSizingBehavior, UniformListScrollHandle, Window,
};
...
pub(crate) fn render_diff_view(
    ...
    let diff_body: AnyElement = if let Some(file) = selected_file {
            ...
            .is_none_or(|lines| lines.is_empty())
        {
            ...
            uniform_list("diff-scroll", item_count, move |range, _window, _cx| {
                range
            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .flex_col()
                        .gap_1()
                        .child(div().text_sm().font_weight(FontWeight::BOLD).child("Diff"))
                        .child(
                            div()
                                .text_xs()
