...
impl ParsedSource {
    ...
    fn extract_let_edges(
        ...
        for ident_name in ident_names {
            if let Some(ident_row) =
                Self::find_ident_row_in_scope(&ident_name, let_row, func_ranges)
            {
    ...
    fn extract_return_edges(
        ...
        for ident_name in returned_idents {
            if let Some(ident_row) =
                Self::find_ident_row_in_scope(&ident_name, return_row, func_ranges)
            {
                edges.push(ExtractedEdge {
                    source: EdgeEndpoint::Row(ident_row),
    ...
    fn extract_call_edges(
        ...
        if let Some(args_node) = node.child_by_field_name("arguments") {
            ...
            for ident_name in arg_idents {
                if let Some(ident_row) =
                    Self::find_ident_row_in_scope(&ident_name, call_row, func_ranges)
                {
                    edges.push(ExtractedEdge {
                        source: EdgeEndpoint::Row(ident_row),
    ...
    fn extract_assignment_edges(
        ...
        for ident_name in right_idents {
            if let Some(ident_row) =
                Self::find_ident_row_in_scope(&ident_name, assign_row, func_ranges)
            {
                edges.push(ExtractedEdge {
                    source: EdgeEndpoint::Row(ident_row),
    ...
    fn containing_function(row: usize, func_ranges: &[(usize, usize)]) -> Option<(usize, usize)> {
        func_ranges
            .iter()
            .find(|&&range| range.0 <= row && row <= range.1)
            .copied()
    }
