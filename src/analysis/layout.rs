...
pub(crate) fn layout_graph(graph: &DataFlowGraph) -> GraphLayout {
    ...
    for layer_idx in 0..=max_layer {
        let nodes_in_layer: Vec<usize> = order.get(&layer_idx).cloned().unwrap_or_default();
        ...
        for (i, &node_id) in nodes_in_layer.iter().enumerate() {
            ...
            let node_type = graph
                ...
                .get(node_id)
                .map(|n| n.node_type)
                .unwrap_or(FlowNodeType::Variable);
    ...
    let total_width = layout_nodes
        .iter()
        .map(|n| n.x + n.width)
        .fold(0.0f32, f32::max)
        + PADDING;
    let total_height = layout_nodes
        .iter()
        .map(|n| n.y + n.height)
        .fold(0.0f32, f32::max)
        + PADDING;
...
fn assign_layers(graph: &DataFlowGraph) -> HashMap<usize, u32> {
    ...
    let adj: HashMap<usize, Vec<usize>> = graph.edges.iter().fold(HashMap::new(), |mut acc, e| {
...
fn order_nodes_in_layers(
    ...
    let node_map: HashMap<usize, FlowNodeType> =
        graph.nodes.iter().map(|n| (n.id, n.node_type)).collect();
